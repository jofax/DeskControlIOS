...
extension UIAlertController {
    ...
    public func addLocationPicker(location: LocationObject? = nil,
...
final public class LocationPickerViewController: UIViewController {
    ...
    public typealias CompletionHandler = (LocationObject?) -> ()
	...
	public var location: LocationObject? {
		didSet {
			if isViewLoaded {
    ...
    func selectLocation(location: CLLocation) {
        ...
        geocoder.reverseGeocodeLocation(location) { response, error in
            if let error = error as NSError?, error.code != 10 { // ignore cancelGeocode errors
                ...
                self.location = LocationObject(name: name, location: location, placemark: placemark)
...
extension LocationPickerViewController: UISearchResultsUpdating {
	...
	func showItemsForSearchResult(_ searchResult: MKLocalSearch.Response?) {
		results.locations = searchResult?.mapItems.map { LocationObject(name: $0.name, placemark: $0.placemark) } ?? []
		results.isShowingHistory = false
		results.tableView.reloadData()
	}
	
	func selectedLocation(_ location: LocationObject) {
