...
struct SearchHistoryManager {
	...
	func history() -> [LocationObject] {
		let history = defaults.object(forKey: HistoryKey) as? [NSDictionary] ?? []
		return history.compactMap(LocationObject.fromDefaultsDic)
	}
	
	func addToHistory(_ location: LocationObject) {
...
extension LocationObject {
	...
	class func fromDefaultsDic(_ dic: NSDictionary) -> LocationObject? {
		...
		return LocationObject(name: dic[LocationDicKeys.name] as? String,
